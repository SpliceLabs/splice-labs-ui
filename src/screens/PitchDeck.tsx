...
function DeckPlaceholder({ deckId, deckLabel }: { deckId: string; deckLabel: string }) {
  return (
    <div className="min-h-[60vh] flex flex-col items-center justify-center gap-6 px-6">
      <span className="font-mono text-label text-accent/40 tracking-splice-ultra uppercase">
      ...
      <div className="flex items-center gap-3 mt-4">
        ...
        <span className="font-mono text-label text-muted-foreground/30 tracking-splice-wide">Coming soon</span>
        <span className="w-8 h-px bg-accent/30" />
      </div>
    </div>
  );
  ...
  return (
    <>
      ...
      <div className="border-b border-surface-border py-8 flex items-center justify-center">
        <span className="font-mono text-label text-accent/40 tracking-splice-ultra uppercase">── Appendices ──</span>
      </div>
        ...
        <div className="container-content flex items-center justify-between h-12">
          ...
          <div className="flex items-center gap-4">
            <button
              ...
              className="font-mono text-eyebrow tracking-splice-wide uppercase border border-surface-border px-2 py-0.5 hover:text-foreground transition-colors"
            ...
            <a
              href="/"
              className="font-mono text-eyebrow text-muted-foreground/40 tracking-splice-wide uppercase hover:text-foreground transition-colors"
            >
              ← Back
            </a>
          </div>
          ...
          {DECKS.map((d) => {
            if ((d as { href?: string }).href) {
              return (
                <a
                  ...
                  className="font-mono text-label tracking-splice-wide uppercase px-4 py-2.5 border-b-2 border-transparent text-muted-foreground hover:text-foreground transition-colors shrink-0"
                >
                  {d.label}
                </a>
              );
            ...
            return (
              <button
                ...
                className={`font-mono text-label tracking-splice-wide uppercase px-4 py-2.5 border-b-2 transition-colors shrink-0 ${
                  activeDeck === d.id
