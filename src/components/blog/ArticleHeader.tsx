...
export function ArticleHeader({
  ...
  return (
    ...
    >
      ...
      <h1 className="font-display text-4xl font-semibold leading-[1.05] tracking-splice-snug md:text-5xl">
        {title}
      </h1>
        ...
        <dl className="flex flex-wrap gap-x-10 gap-y-3">
          {meta.map((m) => (
            <div key={m.label} className="flex flex-col gap-1">
              <dt className="font-mono text-meta uppercase tracking-splice-wide text-blog-text-muted">
                {m.label}
              </dt>
